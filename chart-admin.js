...
window.prepareAdminChartData=function(){
 ...
 for(const item of state.customChartRecords||[])if(!CHART_RECORDS.some(r=>r.id===item.id))CHART_RECORDS.push({...item});
};
...
window.initializeAdminEnhancements=function(){
 ...
 const baseFaculty=renderFaculty;renderFaculty=function(){
  ...
  const liveCollections=['orders','labs','notes','vitals','io','assessments','mar','laborProgress','postpartumRecovery','pphPads','pphMedications','bloodAdministration','surgicalChecklist','surgicalAssessments','messages','notifications','diagnosticFiles','labPanels','audit'];
  ...
  savePatientProfile.onclick=()=>{let extra;try{extra=JSON.parse(additionalPatientDetails.value||'{}');if(!extra||Array.isArray(extra)||typeof extra!=='object')throw new Error();}catch(e){alert('Additional Patient Details must be valid JSON.');return}for(const [key] of profileFields){const value=document.querySelector(`[data-profile-field="${key}"]`).value.trim();patient[key]=key==='allergies'?value.split(',').map(x=>x.trim()).filter(Boolean):key==='weightKg'&&value!==''?Number(value):value;}Object.assign(patient,extra,{id:patient.id});audit('Faculty edited patient profile',activePatientId,'Patient demographics and profile updated');renderFaculty();};
